...
- **PDF only (source-pending)** (current): see the main paper's changelog section for details.
...
**vPDF only (source-pending) supersedes all earlier versions** for current reference. If citing for strict reproducibility of work published before this version, cite the version DOI from the specific Zenodo deposit the earlier work used.
...
*Generated by `create-repo`. The `- **PDF only (source-pending)** (current): see the main paper's changelog section for details.
