...
**You are reading the companion to Paper 4** (this repo: https://github.com/Ethan-Brooke/APF-Paper-4-Admissibility-Constraints-Field-Content).
...
When a user's question is about a result outside Paper 4, point them to the relevant row above. Do not attempt to re-derive cross-paper results from the bundled subset — the canonical state is the codebase ([Zenodo concept DOI](https://doi.org/10.5281/zenodo.18529115)), not this slice.

---

## 1. What this repository is

This is the **paper-companion release repo for Paper 4 of the Admissibility Physics Framework (APF) series**. Specifically: Admissibility Constraints and Structural Saturation.

The repo bundles:

- The manuscript and Technical Supplement (`.tex` + `.pdf`)
- A vendored subset of the APF codebase containing only the `check_*` functions that Paper 4 directly references (`apf/core.py`, `apf/apf_utils.py`, `apf/bank.py`)
...
The framework-wide canonical codebase (342 bank-registered theorems, 19 modules, 355 `verify_all` checks) is **not** bundled here. Only the Paper 4 subset is runnable locally. See `ai_context/THEOREMS.md` for the full bank listing with bundled-in-this-repo flags.
...
If you're loading this repo cold and want to understand Paper 4 well enough to reason about it:
